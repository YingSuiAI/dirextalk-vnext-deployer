...
only reads the fixed local `.dirextalk-deployment-state` directory on Unix and
fails as unsupported on other platforms. Validation and planning remain
cross-platform. There is no deployment execution flag, remote transport,
production migrator, service mutation, provider activation, or remote Connector
